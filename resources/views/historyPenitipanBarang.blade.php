                ...
                <div class="col-md-4 mb-4">
                    ...
                    <div class="card h-100 test ">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text">Tanggal Masuk:
                                {{ Carbon\Carbon::parse($barang->tanggal_masuk)->format('d-m-Y H:i:s') }} </p>
                            <p class="card-text">Tanggal Akhir Penitipan:
                                {{ Carbon\Carbon::parse($barang->tanggal_akhir)->format('d-m-Y H:i:s') }} </p>
                            <p class="card-text">Tanggal Batas Pengambilan Penitipan:
                                </br>{{ Carbon\Carbon::parse($barang->tanggal_batas)->format('d-m-Y H:i:s') }} </p>
                            @if ($barang->tanggal_laku != null)
                                <p class="card-text">Tanggal Laku:
                                    {{ Carbon\Carbon::parse($barang->tanggal_laku)->format('d-m-Y H:i:s') }}</p>
                            @endif
                            @if ($barang->tanggal_ambil == null and $barang->status == 'Diambil')
                                <p class="card-text">Tanggal Diambil: <strong
                                        class="badge text-bg-warning">Pending</strong></p>
                            @elseif($barang->tanggal_ambil != null)
                                <p class="card-text">Tanggal Diambil:
                                    {{ Carbon\Carbon::parse($barang->tanggal_ambil)->format('d-m-Y') }}</p>
                            @endif
